...
class FoodOptionCell: UITableViewCell {
    ...
    let titleLabel: UILabel = {
        let lb = UILabel()
        lb.text = "Seçili ürünler %10 indirimli!"
        lb.scaleFont()
        lb.numberOfLines = 1
    ...
    func manageData(){
        ...
        if let discountPrice = item.discountPrice {
            ...
            discountPriceLabel.text = "₺\(discountPrice.twoDecimalPlaces())"
            discountedContainer.isHidden = false
        }else{
            priceLabel.attributedText = NSAttributedString(string: "₺\(item.price.twoDecimalPlaces())", attributes: [.foregroundColor: color.black])
            discountedContainer.isHidden = true
        }
